	...
	>
					...
					>
						{#if completed}
							<svg width="13" height="13" viewBox="0 0 18 18" fill="none">
								<path d="M4 9l3.5 3.5L14 5" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" />
							</svg>
						{/if}
					</div>
					<span
		...
		{#if completedCount > 0}
			<div class="selection-summary">
				{#each plate.steps as s, i (s.key)}
					{#if selections[s.key]}
						<div class="pill" style:border-color="{accent}40" style:background="{accent}08">
